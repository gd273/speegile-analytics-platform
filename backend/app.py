...
# # AFTER — reads the token's actual "good until" time instead of guessing
# _token_cache = {"access_token": None, "expires_at": 0}
# TOKEN_SAFETY_MARGIN = 60

# def get_superset_access_token(force_refresh=False):
#     now = time.time()
#     if not force_refresh and _token_cache["access_token"] and now < _token_cache["expires_at"] - TOKEN_SAFETY_MARGIN:
#         return _token_cache["access_token"]

#     try:
#         response = requests.post(
#             f"{SUPERSET_URL}/api/v1/security/login",
#             json={"password": SUPERSET_ADMIN_PASSWORD, "provider": "db", "refresh": True, "username": SUPERSET_ADMIN_USERNAME},
#             timeout=10
#         )
#         if response.status_code == 200:
#             token = response.json().get("access_token")

#             # Read the REAL expiry that Superset put inside the token,
#             # instead of assuming a fixed number of minutes.
#             try:
...
#             _token_cache["expires_at"]  = exp_claim if exp_claim else (now + 600)  # safe fallback: 10 min
#             _token_cache["access_token"] = token
#             print(f"DEBUG: Superset token OK (actually valid until {_token_cache['expires_at']})", flush=True)
#             return token
#         else:
#             print(f"DEBUG: Superset login failed — status={response.status_code}, body={response.text[:300]}", flush=True)
#             return None
#     except Exception as e:
#         print(f"Error getting Superset token: {e}", flush=True)
#         return None
        
# def call_superset_api(method, path, **kwargs):
#     """
#     Wraps any Superset API call. If Superset rejects the cached token (401),
#     it automatically fetches a fresh one and retries the SAME request once —
#     so one bad cached token can never cause a lasting outage again.
#     `path` is the part after SUPERSET_URL, e.g. "/api/v1/dashboard/".
#     Any extra requests.get/post arguments (params, json, timeout, etc.) pass through.
#     """
...
#     # Inside call_superset_api, right after headers["Authorization"] = ... , add:
#     headers.setdefault("Content-Type", "application/json")

#     response = requests.request(method, f"{SUPERSET_URL}{path}", headers=headers, **kwargs)

#     if response.status_code == 401:
#         print(f"DEBUG: Superset 401 on {path} — refreshing token and retrying once", flush=True)
#         fresh_token = get_superset_access_token(force_refresh=True)
#         if fresh_token:
...
# # AFTER — if the token gets rejected, get a fresh one and try again automatically;
# # and if it fails for any other reason, actually log why instead of going silent
# def get_all_dashboards_from_superset(access_token, _retried=False):
#     try:
#         query    = {"page": 0, "page_size": 100}
#         response = requests.get(
#             f"{SUPERSET_URL}/api/v1/dashboard/",
...
#             print("DEBUG: Superset rejected the token (401) — getting a fresh one and retrying once", flush=True)
#             fresh_token = get_superset_access_token(force_refresh=True)
#             if fresh_token:
#                 return get_all_dashboards_from_superset(fresh_token, _retried=True)

#         print(f"DEBUG: Superset dashboard list failed — status={response.status_code}, body={response.text[:300]}", flush=True)
#         return []
#     except Exception as e:
#         print(f"DEBUG: get_all_dashboards_from_superset exception: {e}", flush=True)
#         return []
...
# # AFTER — uses the shared helper, so a stale token self-heals here too,
# # and a failure actually gets logged instead of silently skipped
# def get_embedded_dashboard_uuid(filtered_dashboard_list, access_token=None):
#     embedded_dashboards = []
#     if not filtered_dashboard_list:
#         return []
...
#             continue
#         try:
#             response = call_superset_api("GET", f"/api/v1/dashboard/{dashboard_id}/embedded", timeout=10)
#             if response is None:
#                 print(f"DEBUG: No Superset token available for dashboard {dashboard_id}", flush=True)
#                 continue
#             if response.status_code == 200:
#                 result_data = response.json().get("result")
#                 if result_data and "uuid" in result_data:
...
#                     embedded_dashboards.append(filtered_dashboard)
#             else:
#                 print(f"DEBUG: embedded lookup failed for dashboard {dashboard_id} — status={response.status_code}", flush=True)
#         except Exception as e:
#             print(f"DEBUG: embedded lookup exception for dashboard {dashboard_id}: {e}", flush=True)
...
#         # )
#         # AFTER
#         response = call_superset_api("GET", f"/api/v1/dashboard/{dashboard_id}/charts", timeout=15)
#         if response is None:
#             return jsonify({"error": "Superset auth failed"}), 500
...
#         # AFTER
#         dashboard_meta_resp = call_superset_api("GET", f"/api/v1/dashboard/{dashboard_id}", timeout=15)
#         if dashboard_meta_resp is None:
#             return jsonify({"error": "Superset auth failed"}), 500
...
#                 timeout=10
#             )
#             return chart_id, resp.json().get("result", {})
...
#         # AFTER
#         chart_resp = call_superset_api("GET", f"/api/v1/chart/{slice_id}", timeout=10)
...
#         # AFTER
#         data_resp = call_superset_api("POST", "/api/v1/chart/data", json=query_context, timeout=30)
...
#         # )
#         # AFTER
#         resp = call_superset_api("GET", f"/api/v1/dashboard/{dashboard_id}", timeout=10)
#         if resp is None:
#             return jsonify({"error": "Superset auth failed"}), 500
...
#             # AFTER
#             r = call_superset_api("POST", "/api/v1/chart/data", json=payload, timeout=15)
#             if r is None:
#                 return []
...
REDIS_URL                = os.getenv("REDIS_URL", "redis://redis:6379/0")
redis_client              = redis.from_url(REDIS_URL)  # single shared client reused everywhere below
AGGREGATE_CHART_ROW_LIMIT = 10000   # bar/line/pie/etc: capped, browser can't usefully render more
TABLE_CHART_ROW_LIMIT     = 100000  # table/pivot_table/handlebars: business needs full data
SUPERSET_ADMIN_USERNAME  = os.getenv("SUPERSET_ADMIN_USERNAME")
SUPERSET_ADMIN_PASSWORD  = os.getenv("SUPERSET_ADMIN_PASSWORD")
SUPERSET_URL             = os.getenv("SUPERSET_URL")
DATABASE_URL             = os.getenv("APP_DATABASE_URL")
...
def get_superset_access_token(force_refresh=False):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            _token_cache["expires_at"]  = exp_claim if exp_claim else (now + 600)
            _token_cache["access_token"] = token
            print(f"DEBUG: Superset token OK (actually valid until {_token_cache['expires_at']})", flush=True)
            return token
        else:
...
def upload_excel():
    ...
    r        = redis_client
    lock_key = f"upload_lock:{tenant_schema}"
    lock     = r.set(lock_key, "1", nx=True, ex=600)
...
def get_chart_data():
    ...
    try:
        ...
        chart_viz_type = (
            ...
            or (params.get("viz_type") if not raw_context else None)
            or ""
        )
        TABLE_VIZ_TYPES = {"table", "pivot_table", "pivot_table_v2", "handlebars"}
        row_limit_ceiling = (
            TABLE_CHART_ROW_LIMIT if (chart_viz_type or "").lower() in TABLE_VIZ_TYPES
            else AGGREGATE_CHART_ROW_LIMIT
        )
        for q in query_context.get("queries", []):
            q["row_limit"] = max(q.get("row_limit", 0) or 0, row_limit_ceiling)
