...
# ---------------------------------------------------------
# TENANT: shinde_shoes — SALES file
# ---------------------------------------------------------
...
def handle_shinde_shoes(conn, df, tenant_schema, target_table_name, load_id, log_load_error, engine):
    ...
    # ─────────────────────────────────────────────────────────────
    # STEP 4: staging -> dimension tables (this internally also calls
    # sp_process_sales_rowwise() per batch, which resolves dims and
    # writes FactSalesMaster/FactSalesDetail).
    # ─────────────────────────────────────────────────────────────
    print(f"DEBUG [{tenant_schema}]: Calling sp_batch_insert_dummy_to_stagging_to_dim...", flush=True)
    conn.execute(text(
        f'CALL "{tenant_schema}".sp_batch_insert_dummy_to_stagging_to_dim(200000, 1, 0)'
    ...
    # ─────────────────────────────────────────────────────────────
    # STEP 7 (NEW): stock impact — sp_batch_insert_dummy_to_stagging_to_dim
    # above only writes FactSalesMaster/FactSalesDetail; it never posts
    # the sale into stock_transaction / stock_fact_master. That used to
    # be a manual step (CALL sp_process_sales_stock_impact()) — now
    # automated through the single dispatcher, same as Purchase/Inventory.
    # p_load_id is NULL here on purpose: Sales isn't load-scoped —
    ...
    conn.execute(
        ...
        {"load_id": None, "file_type": "sales"}
    )
    print(f"DEBUG [{tenant_schema}]: sp_process_upload(sales) done.", flush=True)

    print(f"DEBUG [{tenant_schema}]: All steps completed successfully.", flush=True)


# ---------------------------------------------------------
# TENANT: shinde_shoes — STOCK / INVENTORY file
# ---------------------------------------------------------

def handle_shinde_shoes_stock(
    conn,
    df,
    tenant_schema,
    target_table_name,
    ...
    try:
        ...
        # ---------------------------------------------------------
        # 1. Convert all Inventory Excel data to strings
        # ---------------------------------------------------------
        ...
        # 3. Add created_by
        # ---------------------------------------------------------
        df["created_by"] = str(user_id) if user_id is not None else ""

        print(
            f"DEBUG [shinde_shoes]: Inventory rows: {len(df)}",
            flush=True
        )

        print(
            f"DEBUG [shinde_shoes]: Inventory columns: "
            f"{list(df.columns)}",
            flush=True
        )
        ...
        print(
            f"DEBUG [shinde_shoes]: Successfully inserted "
            f"{len(df)} rows into "
        ...
        # ---------------------------------------------------------
        # 5. (NEW) Validate -> Process, via the single dispatcher.
        # Previously this staging insert was the LAST step run
        ...
        conn.execute(
            text(f'CALL "{tenant_schema}".sp_process_upload(:load_id, :file_type)'),
            {"load_id": load_id, "file_type": "inventory"}
        )
        print(f"DEBUG [shinde_shoes]: sp_process_upload(inventory) done.", flush=True)

    except Exception as e:

        print(
            f"ERROR [shinde_shoes]: Inventory staging failed: {e}",
            flush=True
        )
...
def handle_shinde_shoes_purchase(conn, df, tenant_schema, target_table_name, load_id, log_load_error):
    ...
    Purchase's staging/processing is load_id-scoped end to end:
        stg_purchase_1 (this upload's raw rows, load_id tagged)
          -> sp_validate_purchase_load(load_id)
          -> stg_purchase_2 (validated, accumulates across loads)
          -> sp_process_purchase_load(load_id)
          -> purchase_fact_master + stock_fact_master

    Previously Purchase had NO dedicated Python handler at all — it fell
    through to the generic "no handler" branch in app.py, which truncated
    + bulk-inserted stg_purchase_1 correctly, but then called whatever
    single procedure_name was configured in public.tenants with ZERO
    arguments. That can never run BOTH load_id-scoped steps Purchase
    actually needs, which is why sp_validate_purchase_load(load_id) and
    sp_process_purchase_load(load_id) had to be called by hand after
    every Purchase upload up to now.

    NOTE: sp_process_upload's 'purchase' branch calls
    sp_process_purchase_load(load_id) — make sure the corrected,
    date-wise version of that procedure (not the stale whole-load
    version) is deployed, and that stock_fact_master's primary key has
    already been migrated to ("SKUKey","LocationKey",last_date), before
    ...
    try:
        ...
        # sp_validate_purchase_load below) is what actually accumulates
        # history across loads, keyed by (load_id, row_no).
        conn.execute(text(
            f'TRUNCATE TABLE "{tenant_schema}"."{target_table_name}" RESTART IDENTITY'
        ))
        print(f"DEBUG [shinde_shoes]: Truncated {target_table_name}.", flush=True)
        ...
        print(
            f"DEBUG [shinde_shoes]: Inserted {len(df)} rows into "
            f"{tenant_schema}.{target_table_name}.",
            flush=True
        )
        ...
        conn.execute(
            text(f'CALL "{tenant_schema}".sp_process_upload(:load_id, :file_type)'),
            {"load_id": load_id, "file_type": "purchase"}
        )
        print(f"DEBUG [shinde_shoes]: sp_process_upload(purchase) done.", flush=True)

    except Exception as e:
        print(f"ERROR [shinde_shoes]: Purchase staging/processing failed: {e}", flush=True)
        log_load_error(conn, load_id, str(e))
        raise
...
# ---------------------------------------------------------
# TENANT: tally_data (Tally Export — sales_vouchers shape)
# ---------------------------------------------------------
def handle_tally_sales(conn, df_original, tenant_schema, target_table_name, load_id, log_load_error):
    import re
    from datetime import datetime
    ...
    for _, row in df.iterrows():
        ...
        insert_data.append({
            ...
            "sales_ac":           clean_numeric(row.get("Sales A/C"), float),
            "discount_on_sales":  clean_numeric(row.get("Discount on Sales"), float),
            "cgst_amount":        clean_numeric(row.get("CGST@"), float),
            "sgst_amount":        clean_numeric(row.get("SGST@"), float),
            "round_off":          clean_numeric(row.get("Round Off"), float),
            "load_id":            load_id,
        })
...
# ---------------------------------------------------------
# TENANT: accrec (Accounts Receivable invoices)
# ---------------------------------------------------------
...
def handle_accrec(conn, df, tenant_schema, target_table_name, load_id, log_load_error):
    ...
    for col in ["product_category", "product_subcategory"]:
        if col in df.columns:
